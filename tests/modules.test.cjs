...
const OURS = ["DZ Indicateurs", "DZ Tableau", "DZ Répartition", "DZ À venir", "DZ Graphique", "DZ Calendrier", "DZ Journal", "DZ Statut", "DZ Disponibilité"];
...
for (const v of views.values()) {
  ...
  if (["DZ Graphique", "DZ Calendrier", "DZ Journal", "DZ Statut", "DZ Disponibilité"].includes(v.template)) for (const [k, f] of Object.entries(cfg)) if (k.startsWith("champ_") && f) assert(tables.get(v.table).has(f), `${v.name} : ${k} « ${f} » absent de ${v.table}`);
